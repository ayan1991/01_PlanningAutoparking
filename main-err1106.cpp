...
#define ParkingMode  VERTICAL

#define RANDOM_ANGLE 10
#define CarLength   410
#define CarWidth    180
...
bool CarCornerCheck(vector<Posture>& objectlist, Posture& carCenter){
	...
	car_corner = RotateCar(carCenter, 130 * MAP_SCALE, 410 * MAP_SCALE, 190 * MAP_SCALE);
...
public:
	...
	Posture GetRandomPointParallel(){
		...
		if (random(1, 100) > 95){
			...
			////Posture guidPoint;
			rnd[0] = goal.x +15+ random(1, 30)* MAP_SCALE;// (790 - random(1, 50))* MAP_SCALE;
			rnd[1] = goal.y +(70+ random(1, 50))* MAP_SCALE;//(300 - random(1,30)) * MAP_SCALE;//
			rnd[2] = pi / 6;
			rnd[3] = -0.2;
...
int main(int argc, char* argv[]){
	...
	while ((loop_time++ < LoopCount)){
		...
		if (ParkingMode == VERTICAL){
			...
			//start.speed = 0;
			//100
			//start.x = random(800, 1900)* MAP_SCALE;
			//start.y =  random(600, 700) * MAP_SCALE;//750 * MAP_SCALE;
			...
			if (start.x > end.x && start.y< (end.y+50)){
				...
				guide1.x = end.x - ParkWidthVer* MAP_SCALE;
				guide1.y = end.y + ParkHightVer / 2* MAP_SCALE;
				guide1.yaw = 0;
				parkingList0 = path_point.Generate(guide1, start, step_size, rho);

			}
		}
		else{
			//paralle
			start.x = 170.5;//random(300, 1300)* MAP_SCALE; //170.5;//
			start.y = 382.5;// random(550, 900) * MAP_SCALE;//382.5;//		
			start.yaw = (random(0,2)-1)* random(0, RANDOM_ANGLE)*0.01744f;// 0;// pi / 8;
			start.speed = 0;
			if ((start.x  < end.x - ( ParkHightPara / 2)* MAP_SCALE) &&
				 (start.y < end.y + ( ParkWidthPara*4)* MAP_SCALE)){
				std::cout << "[ guide1 para] : |  " << endl;
				guide1.x = end.x + ParkHightPara * MAP_SCALE;
				guide1.y = end.y + 2*ParkWidthPara * MAP_SCALE;
				guide1.yaw = pi/8;
		...
		if (ParkingMode == VERTICAL){
			...
			Posture guidPoint;
			guidPoint.x = end.x + 80 * MAP_SCALE;//790
			guidPoint.y = end.y + 100 * MAP_SCALE;//320
			guidPoint.yaw = pi / 6;
			guidPoint.speed = 0.2;
			std::vector<Posture>  parkingList1, parkingList2;
			parkingList2 = path_point.Generate( guidPoint,end, step_size, rho);
			for (int iindex = 0; iindex < parkingList2.size(); ++iindex){
				rrt_Path.path_list_.push_back(parkingList2[iindex]);
			}
		}
		...
		if (show_animation){
			...
			for (int i = 0; i < obstacleList.size(); ++i){
				...
				circle(img, center_obs, 5 * MAP_SCALE, Scalar(0, 255, 255), 5);
			}
			//path show
