...
#define CarWidth    210
#define MAP_SCALE    0.1//1
#define random(a,b) (rand()%(b-a+1)+a)

clock_t start_time, end_time;

const float  step_size = 30;
const float  rho = 420; // turning radius

void get_point(vector<Posture>& objectlist, Posture& carCenter) {
	...
	/*x = carCenter.x + radius * cos(orin);
	y = carCenter.y + radius * sin(orin);*/
	return;
}
...
public:
	...
	void DrawCar(cv::Mat &img ,Posture &PathPoint){
		//start show
		RotatedRect rRect(Point2f(PathPoint.x, PathPoint.y), Size2f(180, 440), (90) + 57.6*PathPoint.yaw);
		//¶¨Òå¾ØÐÎµÄ4¸ö¶¥µã	
		Point2f vertices[4]; 
		//¼ÆËã¾ØÐÎµÄ4¸ö¶¥µã	
		rRect.points(vertices);   
		for (int i = 0; i < 4; i++)	{
			line(img, vertices[i], vertices[(i + 1) % 4], Scalar(0, 0, 0), 1.5);
		}
	}
};
...
void InitObs(Posture &start, Posture &end, vector <Posture>& obstacleList){
	...
	if (ParkingMode == VERTICAL){
		//VERTICAL
		for (int i = 0; i < 400 * MAP_SCALE; ++i){
			node.x = 500 * MAP_SCALE;
			node.y = 950 * MAP_SCALE - i;
			node.yaw = 25;
			i = i + 15;
			obstacleList.push_back(node);
		}
		for (int i = 0; i <400 * MAP_SCALE; ++i){
			node.x = 800 * MAP_SCALE;
			node.y = 950 * MAP_SCALE - i;
			node.yaw = 25;
			i = i + 15;
			obstacleList.push_back(node);
		}
		for (int i = 0; i < 300 * MAP_SCALE; ++i){
			node.x = 500 * MAP_SCALE + i;
			node.y = 950 * MAP_SCALE;
			node.yaw = 25;
			i = i + 15;
			obstacleList.push_back(node);
		}
		for (int i = 0; i < 1460 * MAP_SCALE; ++i){
			node.x = i;
			node.y = 50 * MAP_SCALE;
			node.yaw = 25;
			i = i + 15;
		...
		for (int i = 0; i < 500 * MAP_SCALE; ++i){
			node.x = i;
			node.y = 550 * MAP_SCALE;
			node.yaw = 25;
			i = i + 15;
			obstacleList.push_back(node);
		}
		for (int i = 800; i < 1500 * MAP_SCALE; ++i){
			node.x = i;
			node.y = 550 * MAP_SCALE;
			node.yaw = 25;
			i = i + 15;
		...
		for (int i = 0; i < 1500 * MAP_SCALE; ++i){
			node.x = i;
			node.y = 1250 * MAP_SCALE;
			node.yaw = 25;
			i = i + 15;
			obstacleList.push_back(node);
		}
	...
	if (ParkingMode == PARALLEL){
		...
		//bianxian
		for (int i = 0; i < 750 * MAP_SCALE; ++i){
			node.x = 350 * MAP_SCALE + i;
			node.y = 10 * MAP_SCALE;
			node.yaw = 50;
			i = i + 15;
			obstacleList.push_back(node);
		...
		for (int i = 1100 * MAP_SCALE; i < 1500 * MAP_SCALE; ++i){
			node.x = i;
			node.y = 280 * MAP_SCALE;
			node.yaw = 50;
			i = i + 15;
...
int main(){
	...
	std::vector<Posture> path_list, obstacleList;
	ReedsSheppPathGenerator  path_point;
	start_time = clock();
	...
	{
	case VERTICAL:
		start.x = 100 * MAP_SCALE;
		start.y = 350 * MAP_SCALE;
		start.yaw = -pi / 8;
		...
		minAreaX = 300 * MAP_SCALE;// 
		maxAreaX = 950 * MAP_SCALE;// 
		minAreaY = 100 * MAP_SCALE;// 
		maxAreaY = 650 * MAP_SCALE;// 
		break;
	...
	case PARALLEL:
		...
		minAreaX = 400 * MAP_SCALE;//minArea - 50;
		minAreaY = 50 * MAP_SCALE;//minArea - 50;

		maxAreaX = 1400 * MAP_SCALE;//maxArea + 50;
		maxAreaY = 800 * MAP_SCALE;//maxArea + 50;

		break;
	case INCLINE:
		start.x = 100 * MAP_SCALE;
		start.y = 400 * MAP_SCALE;
		start.yaw = -pi / 10;

		end.x = 625 * MAP_SCALE;
		end.y = 610 * MAP_SCALE;
		end.yaw = -pi / 2;
		minAreaX = 200 * MAP_SCALE;//minArea - 50;
		maxAreaX = 1400 * MAP_SCALE;//maxArea + 50;
		minAreaY = 200 * MAP_SCALE;//minArea - 50;
		maxAreaY = 1400 * MAP_SCALE;//maxArea + 50;
		break;
	default:
		break;
	}
	...
	if (show_animation){
		...
		cv::Mat img(1000 * MAP_SCALE, 1500 * MAP_SCALE, CV_8UC3, Scalar(192, 192, 192));//Scalar(255, 255, 255)
		//show random point area
		cv::rectangle(img, cvPoint(minAreaX, minAreaY), cvPoint(maxAreaX, maxAreaY), cvScalar(0, 255, 100), 1, 4, 0);
		...
		for (int i = 0; i < path_list.size(); ++i){
			//DrawCar
			if (PointCarCollisionCheck(obstacleList,path_list[i])!=true){
				rrt_Path.DrawCar(img, path_list[i]);// ÎÞÅö×²
			}
			//DrawCar path
